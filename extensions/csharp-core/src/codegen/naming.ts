...
function changeCase(name: string, lower: boolean): string {
  if (name && name.length > 0) {
    if (
      ...
      (name.length == 2 &&
        name.toLowerCase() !== "to" &&
        isUpper(name[0]) == isUpper(name[1]))
    ) {
      name = lower ? name.toLowerCase() : name.toUpperCase();
    } else {
      name =
        (lower ? name[0].toLowerCase() : name[0].toUpperCase()) + name.slice(1);
    }
  }
  return name;
}
...
export function method(name: string, isAsync: boolean) {
  return pascalCase(name) + (isAsync ? "Async" : "");
}
...
export function interfaceName(name: string) {
  return "I" + type(name);
}
