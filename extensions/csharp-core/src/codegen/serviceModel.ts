...
} from "@microsoft.azure/autorest.codemodel-v3/dist/code-model/components";
import { Info } from "@microsoft.azure/autorest.codemodel-v3/dist/code-model/info";
import { Extensions } from "@microsoft.azure/autorest.codemodel-v3/dist/code-model/extensions";
import {
...
export async function create(project: IProject): Promise<IServiceModel> {
  ...
  // for (const [name, model] of Object.entries(project.state.model.schemas)) {
  //   if (!model) {
  //     continue;
  //   }
  //   if (model.external) {
    ...
    operation.parameters.splice(
      ...
      new HttpOperationParameter(
        ...
        ImplementationLocation.Method,
        {
          ...
          explode: false,
          allowReserved: false,
          schema: new Schema("Azure.Core.Pipeline.HttpPipeline", {
            type: JsonType.Object
            //details.namespace: "Azure.Core.Pipeline",
          })
        }
      )
    );
...
//       project,
//       project.swagger.parameters[name],
...
//       project,
//       name,
//       project.swagger.definitions[name],
...
//       code,
//       name,
//       project.swagger.responses[name],
//       `swagger.responses['${name}']`
//     );
//     if (response.body) {
...
//         project.logger.warn(`Ignoring redefined ${model.name}`);
//       }
//     } else {
...
function createServiceInfo(project: IProject): IServiceInfo {
  ...
  //const swaggerVersion = project.state.modelrequired(() => project.swagger.swagger);
  //if (swaggerVersion !== `2.0`) {
  //  throw `Can only process swagger 2.0, not ${swaggerVersion}`;
  //}
  ...
  todo(
    project,
    () => project.state.model.info.contact,
    `project.state.model.info.contact`
  );
  todo(
    project,
    () => project.state.model.info.termsOfService,
    `project.state.model.info.termsOfService`
  );
  ...
  //    optional(
  //      () => project.state.model.info.extensions[`x-ms-code-generation-settings`].header
  //    )
  //  )
  //);
  //if (license !== "MIT") {
  //  throw `Only the MIT license is supported`;
  //}

  // Process schemes
  //const schemes = project.state.model.schemas;
  //if (schemes.length != 1 || schemes[0] != `https`) {
  //  throw `Only HTTPS is supported for  project.state.model.schemes`;
  //}

  // Process produces + consumes
  //const consumes = required(() => project.state.model.consumes);
  //if (consumes.length != 1 || consumes[0] != `application/xml`) {
  //  throw `Only application/xml is supported for project.state.model.consumes`;
  //}
  //const produces = required(() => project.state.model.produces);
  //if (produces.length != 1 || produces[0] != `application/xml`) {
  //  throw `Only application/xml is supported for project.state.model.produces`;
  //}
  ...
  //const host = project.state.model.servers.
  ...
  //   serviceHost = {
  //     template: template.parse(<string>(
  //       required(() => host.hostTemplate, location)
  //     )),
  //     parameters: params,
  ...
  if (project.state.model.servers.length == 1) {
    serverHost = project.state.model.servers[0];
  } else {
    throw "Found more then one server in project.state.model.servers, when only expecting one.";
  }
  if (project.state.model.security) {
  ...
  // Create the info
  const title = project.state.model.info.title;
  return {
    title: title,
...
//   if (at === `formData`) {
//     throw `formData is not supported for ${location}.in`;
//   }

//   const reqd = optional(() => swagger.required, at === `path`);
//   if (at === `path` && !reqd) {
//     throw `${location}.required must be true for path parameters`;
//   }

//   const clientName = optional(() => swagger[`x-ms-client-name`], name);
//   const skipUrlEncoding = optional(
//     () => swagger[`x-ms-skip-url-encoding`],
//     false
//   );

//   const parameterLocation: string | undefined =
//     swagger[`x-ms-parameter-location`];
...
//     unsupported(
//       () => grouping.postfix,
//       `${location}['x-ms-parameter-grouping'].postfix`
//     );
//   }
...
//   if (
//     (swagger.type === "object" || swagger.type === undefined) &&
//     swagger.format !== `file` &&
...
//     project.logger.warn(
//       `${location}.type should be object, not ${swagger.type}`
//     );
//   }

//   // Get the properties
//   const properties: IProperties = {};
//   for (const [name, def] of <[string, any]>(
//     Object.entries(swagger.properties || {})
//   )) {
//     unsupported(
//       () => def[`x-ms-client-flatten`],
...
//     project.logger.warn(
//       `${location}.type should be string, not ${swagger.type} for enum`
//     );
//   }

//   let modelAsString: boolean | undefined = undefined;
//   let values: IEnumValue[];
//   let originalValues = <IEnumValue[]>(
//     (swagger.enum || []).map((v: any) => ({ value: v }))
//   );
...
//   const customSerialization = values.some(
//     v => v.value !== naming.enumField(v.name || v.value)
//   );
...
//   let dictionaryPrefix = <string | undefined>(
//     swagger[`x-ms-header-collection-prefix`]
//   );
...
//       project,
//       "__dictionary__",
//       swagger.additionalProperties || { type: "string" },
//       `${location}.additionalProperties`
//     );
//     type = "dictionary";
//   }
...
//   if (type === `object`) {
//     project.logger.warn(`${location}.type should not be object`);
//   }
...
// function eachReachableObject(
//   type: IModelType,
//   fn: (t: IObjectType) => void,
//   stop?: (t: IObjectType) => boolean
// ): void {
...
//   eachReachableObject(type, t => (t.serialize = true), t => !t.serialize);
// }

// // Mark an object hierarchy for deserialization
// function enableDeserialization(type: IModelType) {
//   eachReachableObject(type, t => (t.deserialize = true), t => !t.deserialize);
// }
...
//   const headers: IHeaders = createHeaders(
//     project,
//     swagger.headers,
//     `${location}['${code}'].headers`
//   );
...
//       optional(() => swagger[`x-ms-schema-client-name`], `Body`)
//     ), // TODO: switch from 'Body' to body.name?
//     headers,
...
function createService(project: IProject, info: IServiceInfo): IService {
  ...
  const service: IService = {
    ...
    name: naming.type(name),
    namespace: naming.namespace(namespace),
    extensionsName: naming.type(info.extensionsName),
    groups: {},
    operations: project.state.model.http.operations
  };

  // // Process the paths
  // for (const [path, def] of <[string, any]>(
  //   Object.entries(optional(() => project.swagger.paths, {}))
  // )) {
  //   addOperations(
  //     project,
  //     service,
  //     path,
  //     def,
  //     `project.swagger.paths['${path}']`
  //   );
  // }
  // for (const [path, def] of <[string, any]>(
  //   Object.entries(optional(() => project.swagger[`x-ms-paths`], {}))
  // )) {
  //   addOperations(
  //     project,
  //     service,
  //     path,
  //     def,
  //     `project.swagger['x-ms-paths']['${path}']`
  //   );
  // }
...
//       if (!operations) {
//         operations = service.groups[op.group] = {};
//       }
//       operations[name] = op;
//     } else {
...
//       consumes[0] != `application/octet-stream`)
//   ) {
//     throw `Only application/xml and application/octet-stream are supported for swagger.consumes, not ${consumes.join(
//       ", "
//     )} (in ${location})`;
//   }
...
//   ) {
//     throw `Only application/xml and application/octet-stream are supported for swagger.produces, not ${produces.join(
//       ", "
//     )} (in ${location})`;
//   }
...
//       p => !isEnumType(p.model) || !p.model.constant
//     ),
...
//     optional(
//       () => def[`x-ms-response-client-name`],
//       `${group ? group + " " : ""}${operationId}Result`
//     )
//   );
//   const responses: IResponses = {};
//   for (const [code, op] of <[string, any]>Object.entries(def.responses || {})) {
//     responses[code] = createResponse(
//       project,
//       code,
//       responseName + " " + code,
//       op,
...
//     );
//   }
//   const responseGroup = getOperationResponse(
//     project,
//     responses,
//     responseName,
//     location
...
//   if (info.host) {
//     (<IParameter[]>Object.values(info.host.parameters)).forEach(
...
//     other.model = createResponseType(
//       other,
//       `${defaultName}Failure${other.code}`,
//       `${location}['${other.code}']`
//     );
//   }
...
//   if (defaultResponse) {
//     defaultResponse.model = createResponseType(
//       defaultResponse,
//       `${defaultName}Failure`,
//       `${location}['${defaultResponse.code}']`
//     );
...
//       break;
//     case 1:
//       model = successes[0].model = createResponseType(
//         successes[0],
//         defaultName,
//         `${location}['${successes[0].code}']`
//       );
//       break;
//     default:
//       const mergedResponses = successes.reduce(mergeResponses);
//       model = createResponseType(
//         mergedResponses,
//         defaultName,
//         `${location}[<<merging ${successes.map(s => s.code).join(", ")}>>]`
//       );
...
//       b.body &&
//       a.body.type !== "array" &&
//       (!isPrimitiveType(a.body) ||
//         !isPrimitiveType(b.body) ||
//         a.body.type !== b.body.type)
//     ) {
//       throw `Cannot merge incompatible schemas for ${a.body.type} and ${
//         b.body.type
//       }`;
//     }
